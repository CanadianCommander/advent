...
console.log(processCommands(inputLines, root));
console.log("Sum of dirs under 100,000 is ", sumUnder(root, 100000));

// ==================================================

function sumUnder(currNode: Node, cutoff: number, sum: number = 0): number {
  if (currNode.type != NodeType.Dir) {
    return sum;
  }

  const currSize = currNode.size;
  if (currSize <= cutoff) {
    sum += currSize;
  }

  sum += (currNode as Dir).children.reduce(
    (accSum, child) => sumUnder(child, cutoff, accSum),
    0
  );
